...
- Seed the demo with a short decision-grade adoption script.
...
- Use a self-contained, no-scroll homepage/demo template and quieter token-faithful docs pages.
...
1. First-time visitor lands directly inside the live run surface, answers a seeded decision-grade question, sees the artifact trail change, and understands the browser/filesystem boundary without scrolling.
...
4. Visitor moves from the demo into the separate overview and docs pages without losing the visual connection to the run UI.
